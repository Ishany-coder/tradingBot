...
# THE model is the NAIVE momentum rank (no ML). The ablation (ABLATION.md) showed
# the trained ensemble beats this baseline in only 48% of resamples — a coin
# flip — so per the pre-registered criterion ("if the ML doesn't beat the naive
# baseline, delete it") the trained stack is not deployed. Benefits: identical
# measured performance, no 20-minute retrains (live cycles are seconds), no seed
